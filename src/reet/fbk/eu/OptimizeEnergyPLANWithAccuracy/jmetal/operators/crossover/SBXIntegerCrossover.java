...
package reet.fbk.eu.OptimizeEnergyPLANWithAccuracy.jmetal.operators.crossover;
 ...
 * solutions.
 */
public class SBXIntegerCrossover extends Crossover {
	/**
	 ...
	 */
	public SBXIntegerCrossover(HashMap<String, Object> parameters) {
		super(parameters);
			...
			for (i = 0; i < numberOfVariables; i++) {
				...
				if (PseudoRandom.randDouble() <= 0.5) {
					if (java.lang.Math.abs(valueX1 - valueX2) > EPS) {
						...
						if( ((int)y1+(int)y2) % 2 ==0 )
						{
							average = ((int)y1+(int)y2)/2;
						}else{
							int randForAvg = PseudoRandom.randInt(0,1);
							if(randForAvg == 0)
								average = (int) Math.ceil(((int)y1+(int)y2)/2.0 );  
							else
								average = (int) Math.floor(((int)y1+(int)y2)/2.0 );
						}
						
						double [][] distribution;
						distribution = calculateTheProbabilityDistribution((int)y1,(int)y2,beta);
						
						int pos=Arrays.binarySearch(distribution[2], rand);
						if(pos<=0)
							pos = -1*(Arrays.binarySearch(distribution[2], rand)+1);
						
						c1 =  0.5 * ((y1 + y2) - distribution[0][pos+1] * (y2 - y1));
	...
	public double[][] calculateTheProbabilityDistribution(int p1, int p2,
		...
		int totalColumn = (int) (betaMax / betaStep);
		double distribution[][] = new double[3][totalColumn + 1];

		distribution[0][1] = 1 * betaStep;
		distribution[1][1] = 0.5 * Math.pow(distribution[0][0],
				(distributionIndex_ + 1));
		distribution[2][1] = distribution[0][0];
		...
		for (int i = 2; i <= totalColumn; i++) {
			...
			if (i * betaStep <= 1.0 && trackBetaForFirstPart) {
				distribution[1][i] = 0.5
						* (Math.pow(distribution[0][i], (distributionIndex_ + 1))
						- Math.pow(distribution[0][i-1], (distributionIndex_ + 1)) );
				distribution[2][i] = distribution[1][i] +  distribution[2][i-1]; 
			} else{
				distribution[1][i] = 0.5
