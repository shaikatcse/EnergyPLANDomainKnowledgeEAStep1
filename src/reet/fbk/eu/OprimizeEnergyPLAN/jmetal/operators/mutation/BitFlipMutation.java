package reet.fbk.eu.OprimizeEnergyPLAN.jmetal.operators.mutation;
...
public class BitFlipMutation extends Mutation {
	...
	private static final List VALID_TYPES = Arrays.asList(
			...
			IntSolutionType.class, BinaryIntSolutionType.class);
	...
	public void doMutation(double probability, Solution solution)
		...
		try {
			if ((solution.getType().getClass() == BinarySolutionType.class)
					...
					|| (solution.getType().getClass() == BinaryIntSolutionType.class)) {
				...
				for (int i = 0; i < solution.getDecisionVariables().length; i++) {
					...
					if (solution.getType().getClass() == BinaryIntSolutionType.class) {
