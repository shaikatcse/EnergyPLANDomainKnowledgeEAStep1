package reet.fbk.eu.OprimizeEnergyPLAN.jmetal.operators.crossover;
			...
			for (i = 0; i < numberOfVariables; i++) {
				...
				if (PseudoRandom.randDouble() <= 1.0 /* by shaikat, should be 0.5 */) {
					if (java.lang.Math.abs(valueX1 - valueX2) > EPS) {
						...
						/*if (((int) y1 + (int) y2) % 2.0 == 0) {
							average = ((int) y1 + (int) y2) / 2;
						} else {
							int randForAvg = PseudoRandom.randInt(0, 1);
							if (randForAvg == 0)
								average = (int) Math
										.ceil(((int) y1 + (int) y2) / 2.0);
							else
								average = (int) Math
										.floor(((int) y1 + (int) y2) / 2.0);
						}*/
	...
	public double[][] calculateTheProbabilityDistribution(int p1, int p2,
		...
		if((int) distance % 2 == 0){
			for (int i = 1; i < totalColumn; i++) {
				 ...
				 * (-distributionIndex_ - 1)) + Math.pow( 1, (-distributionIndex_ -
				 * 1))) / area; distribution[2][i] = distribution[1][i] +
				 * distribution[2][i - 1]; trackBetaForFirstPart = false; }
				 */
				...
				if (localLb < 1.0 && localUb <= 1.0) {
					distribution[1][i] = 0.5
							* (Math.pow(localUb, (distributionIndex_ + 1)) - Math
									.pow(localLb, (distributionIndex_ + 1))) / area;
					...
					distribution[1][i] = 0.5
							* (Math.pow(1, (distributionIndex_ + 1))
									- Math.pow(localLb, (distributionIndex_ + 1))
									+ -Math.pow(localUb, (-distributionIndex_ - 1)) + Math
										.pow(1, (-distributionIndex_ - 1))) / area;
		...
		distribution[2][totalColumn] = distribution[1][totalColumn] + distribution[2][totalColumn - 1];
