...
/*
 * PP and boiler capacity is removed from decision variables
 */
public class EnergyPLANProblemAalborg2ObjectivesWith1EnergyPLANEvolutionForThread extends
		Problem {
	 ...
	 */
	public EnergyPLANProblemAalborg2ObjectivesWith1EnergyPLANEvolutionForThread(
			String solutionType, String path) {
	...
	//	lowerLimit_[2] = 0.0;
		...
		for (var = 2; var < numberOfVariables_ ; var++) {
	...
	public void evaluate(Solution solution) throws JMException {
		...
		String energyPLANrunCommand = ".\\EnergyPLAN_SEP_2013\\EnergyPLAN.exe -i "
				...
				+ "-m " + "\".\\"+ pathOfResultFile+"\\modification.txt" + "\"" +
				" -ascii " +  "\".\\" +pathOfResultFile+"\\result.txt" + "\"";
		try {
			// Process process = new
			...
			process.waitFor();
			process.destroy();
			EnergyPLANFileParse epfp = new EnergyPLANFileParse(pathOfResultFile+"\\result.txt" /*".\\result.txt"*/);
			energyplanmMap = epfp.parseFile();
			new File(pathOfResultFile+"\\result.txt").delete();
			new File(pathOfResultFile+"\\modification.txt").delete();
			...
			if (solution.getDecisionVariables()[0].getValue() > maximumPP) {
				...
				try {
					...
					process.waitFor();
					process.destroy();
					epfp = new EnergyPLANFileParse(pathOfResultFile+"\\result.txt" /*".\\result.txt"*/);
					energyplanmMap = epfp.parseFile();
					new File(pathOfResultFile+"\\result.txt").delete();
					new File(pathOfResultFile+"\\modification.txt").delete();
	...
	void writeModificationFile(Solution solution) throws JMException {
		...
		// wind
		double wind = solution.getDecisionVariables()[2].getValue();
		// off-shore wind
		double offShoreWind = solution.getDecisionVariables()[3].getValue();
		// PV
		double PV = solution.getDecisionVariables()[4].getValue();
		...
		try {
			...
			File file = new File(pathOfResultFile+"\\modification.txt");
			if (file.exists()) {
				file.delete();
