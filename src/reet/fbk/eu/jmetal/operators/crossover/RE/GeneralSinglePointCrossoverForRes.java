 ...
 */
public class GeneralSinglePointCrossoverForRes extends Crossover {
	/**
	...
	private SinglePointCrossoverFavorMaximizationOfRes singlePointCrossoverFavorMaximizationOfRes;
	private SinglePointCrossoverFavorMaximizationOfPP singlePointCrossoverFavorMaximizationOfPP;
	...
	public Solution[] doCrossover(double probability, Solution parent1,
		...
		/*
		 * Solution[] offSpring = new Solution[2]; offSpring[0] = new
		 * Solution(parent1); offSpring[1] = new Solution(parent2); try { if
		 * (PseudoRandom.randDouble() < probability) { if
		 * ((parent1.getType().getClass() == BinarySolutionType.class) ||
		 * (parent1.getType().getClass() == BinaryRealSolutionType.class) ||
		 * (parent1.getType().getClass() == BinaryIntSolutionType.class))
		 * 
		 * { //1. Compute the total number of bits int totalNumberOfBits = 0;
		 * for (int i = 0; i < parent1.getDecisionVariables().length; i++) {
		 * totalNumberOfBits += ((Binary)
		 * parent1.getDecisionVariables()[i]).getNumberOfBits(); }
		 * 
		 * //2. Calculate the point to make the crossover int crossoverPoint =
		 * PseudoRandom.randInt(0, totalNumberOfBits - 1);
		 * 
		 * //3. Compute the encodings.variable containing the crossoverPoint bit
		 * int variable = 0; int acountBits = ((Binary)
		 * parent1.getDecisionVariables()[variable]).getNumberOfBits();
		 * 
		 * while (acountBits < (crossoverPoint + 1)) { variable++; acountBits +=
		 * ((Binary)
		 * parent1.getDecisionVariables()[variable]).getNumberOfBits(); }
		 * 
		 * //4. Compute the bit into the selected encodings.variable int diff =
		 * acountBits - crossoverPoint; int intoVariableCrossoverPoint =
		 * ((Binary) parent1.getDecisionVariables()[variable]).getNumberOfBits()
		 * - diff ;
		 * 
		 * //5. Make the crossover into the gene; Binary offSpring1, offSpring2;
		 * offSpring1 = (Binary)
		 * parent1.getDecisionVariables()[variable].deepCopy(); offSpring2 =
		 * (Binary) parent2.getDecisionVariables()[variable].deepCopy();
		 * 
		 * for (int i = intoVariableCrossoverPoint; i <
		 * offSpring1.getNumberOfBits(); i++) { boolean swap =
		 * offSpring1.bits_.get(i); offSpring1.bits_.set(i,
		 * offSpring2.bits_.get(i)); offSpring2.bits_.set(i, swap); }
		 * 
		 * offSpring[0].getDecisionVariables()[variable] = offSpring1;
		 * offSpring[1].getDecisionVariables()[variable] = offSpring2;
		 * 
		 * //6. Apply the crossover to the other variables for (int i = 0; i <
		 * variable; i++) { offSpring[0].getDecisionVariables()[i] =
		 ...
		 * }
		 * 
		 * //7. Decode the results for (int i = 0; i <
		 * offSpring[0].getDecisionVariables().length; i++) {
		 * if(parent1.getType().getClass() == BinaryIntSolutionType.class){
		 * 
		 * ((BinaryInt) offSpring[0].getDecisionVariables()[i]).decode();
		 * if(((BinaryInt) offSpring[0].getDecisionVariables()[i]).value_ >
		 * ((BinaryInt) offSpring[0].getDecisionVariables()[i]).upperBound_){
		 * ((BinaryInt)
		 * offSpring[0].getDecisionVariables()[i]).setValue(((BinaryInt)
		 * offSpring[0].getDecisionVariables()[i]).upperBound_); }else
		 * if(((BinaryInt) offSpring[0].getDecisionVariables()[i]).value_ <
		 * ((BinaryInt) offSpring[0].getDecisionVariables()[i]).lowerBound_){
		 * ((BinaryInt)
		 * offSpring[0].getDecisionVariables()[i]).setValue(((BinaryInt)
		 * offSpring[0].getDecisionVariables()[i]).lowerBound_);
		 * 
		 * } ((BinaryInt) offSpring[1].getDecisionVariables()[i]).decode();
		 * if(((BinaryInt) offSpring[1].getDecisionVariables()[i]).value_ >
		 * ((BinaryInt) offSpring[1].getDecisionVariables()[i]).upperBound_){
		 * ((BinaryInt)
		 * offSpring[1].getDecisionVariables()[i]).setValue(((BinaryInt)
		 * offSpring[1].getDecisionVariables()[i]).upperBound_); }else
		 * if(((BinaryInt) offSpring[1].getDecisionVariables()[i]).value_ <
		 * ((BinaryInt) offSpring[1].getDecisionVariables()[i]).lowerBound_){
		 * ((BinaryInt)
		 * offSpring[1].getDecisionVariables()[i]).setValue(((BinaryInt)
		 * offSpring[1].getDecisionVariables()[i]).lowerBound_);
		 * 
		 * }
		 * 
		 * }
		 * 
		 * ((Binary) offSpring[0].getDecisionVariables()[i]).decode(); ((Binary)
		 * offSpring[1].getDecisionVariables()[i]).decode();
		 * 
		 * } } // Binary or BinaryReal else { // Integer representation int
		 * crossoverPoint = PseudoRandom.randInt(0, parent1.numberOfVariables()
		 * - 1); int valueX1; int valueX2; for (int i = crossoverPoint; i <
		 * parent1.numberOfVariables(); i++) { valueX1 = (int)
		 * parent1.getDecisionVariables()[i].getValue(); valueX2 = (int)
		 * parent2.getDecisionVariables()[i].getValue();
		 * offSpring[0].getDecisionVariables()[i].setValue(valueX2);
		 * offSpring[1].getDecisionVariables()[i].setValue(valueX1); } // for }
		 * // Int representation } } catch (ClassCastException e1) {
		 * Configuration
		 * .logger_.severe("SinglePointCrossover.doCrossover: Cannot perfom " +
		 * "SinglePointCrossover"); Class cls = java.lang.String.class; String
		 * name = cls.getName(); throw new JMException("Exception in " + name +
		 * ".doCrossover()"); }
		 */
		...
		if (random <= 15) {
			...
			offSpring = singlePointCrossoverForRes.doCrossover(
					probability, parent1, parent2);
		}
		return offSpring;
	}// doCrossover
