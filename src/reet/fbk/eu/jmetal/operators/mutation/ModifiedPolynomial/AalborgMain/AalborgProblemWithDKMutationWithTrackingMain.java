...
long seed [] = {	
	...
	String path = "mutationResults\\WithTrack\\NSGAII_DK";
		int numberOfRun=30;
		for (int i = 0; i < numberOfRun; i++) {
			...
			algorithm = new NSGAIIForDK(problem, seed[i], path);
...
//seeds for run 1 to run 30 -> (matched with with SPEA2 normal run (reet.fbk.eu.jmetal.initialization.AalborgProblemWithNormalInitilizationMain)		
		...
		String path = "mutationResults\\WithTrack\\SPEA2_DK";
		int numberOfRun=30;
		for (int i = 0; i < numberOfRun; i++) {
			...
			indicators = new QualityIndicator(problem,
					".\\mutationResults\\ParetoFront\\WithoutRM\\With_mutation_pr_0.1\\mergeFUN.pf");
			
			parameters = new HashMap();
						
			algorithm = new SPEA2ForDK(problem, seed[i], path);
			// algorithm = new ssNSGAII(problem);
...
public class AalborgProblemWithDKMutationWithTrackingMain {
