...
This report is about the chromosome representation and design of a mutation based on domain knowledge. The report also reviews the quality indicators for measuring a multi-objective optimization evolutionary algorithm. 
...
In the next few sections, the problems of representation is discussed and a repair function is developed to deal with the problem.
...
Each fuel share value represents a relative share with respect to the use of other fuels. 
As an example, if 0.5 and 0.5 is given for coal and oil, EnergyPLAN\footnote{It is assumed that EnergyPLAN is already introduced.} try to use same amount of coal and oil to meet the demand. 
...
The main concept of repair function is that repair the genes\footnote{Each individual may contains several genes}. 
...
In our case, most of the time, decision maker are not interested in exact percentage (e.g.: 33\% coal, 23\% oil and 44\% natural gas) of the uses of fuels. 
...
Each gene to represent fuel share has a limit between 0 to 1. 
...
The 2\textsuperscript{nd} row is the relative shares calculated using equation \eqref{eq:Fuel Share}.
...
Next, we present an algorithm to repair the gene value according to the step size given by a decision maker \ref{alg:repair-fuel-gene}. 
...
The adjustment is done by using the equation \ref{eq:Representation of capacity design variables2}. 
...
Mutation is one of the key operators in EA basically mimic the idea of mutation of nature. 
...
In a single mutation process, only one mutation is applied within three mutations.  
First mutation is designed to favours renewable sources i.e,  increase their capacity and use cleaner fuel, other mutation is designed to do completely the opposite (i.e.: favours conventional sources and low cost fuel). 
...
Without the use of a traditional mutation approach we may lead the search toward extreme solutions. 
In that case, we will lose interesting trade-off solutions for the decision makers. 
The selection of one mutation among three mutations depends on value of a random number over uniformly distributed probability distribution.
Fig. \ref{fig:mutation flowchart} illustrates the overall mutation process ($P_{REFM}$ is the probability of applying the renewable energy favour mutation and $P_{CEFM}$ is the probability of conventional energy favour mutation).
...
\includegraphics[scale=0.6]{./figures/mutation_flowchart.pdf}
...
Traditional mutation operator makes a change to the individual so that the value of genes \footnote{An individual may consist of several genes.} may be decreased or increased. 
Most of the time, a probability distribution is used to generate the new value of a gene e.g., polynomial mutation, normal mutation \cite{Deb_2001}. 
In that case, the gene may fall both in the upper side (value increased) or lower side (value decreased) of the gene (e.g.: anywhere within -2 to 8, in the left figure of fig. \ref{fig:truncated pdf and cdf}).
But to favour renewable sources, we want the mutation operator should make changes to some chosen genes so that the new gene value can only be increased (e.g.: anywhere within gV to gUB in the left figure of fig.\ref{fig:truncated pdf and cdf}).
...
And if the gene represents the capacity of a renewable source, the mutation operator should generate a new gene only within the range of 1250 to 2000. 
If we want to favour conventional sources, the mutation should produce a new gene only within the range 0 to 1250. 

To design the renewable or conventional favour mutation, we use \textit{normal probability  distribution}\cite{wiki:normal-distribution}. 
...
We will have a normal distribution centred on the gene value of an individual (mean, $\mu$ is set equal to gene value and standard deviation, $\sigma$ is set to $d/3$, where $d=|geneValue - geneUpperValue$ or $geneLowerValue|$). 
...
On the other hand, the blue distribution shows the truncated pdf within the range of $gV$ to $gUB$.
...
%      \EndFor
	...
	\State{Draw a random number uniformly distributed over 0 to 1}
...
The mutation that favour conventional energy is the same as the previous one except the choice of favouring decision variables. 
But process that is applied on the variables is the same.

The table \ref{table:variables-in-mutation} shows the variables that are increased or decreased according to the mutation status.
