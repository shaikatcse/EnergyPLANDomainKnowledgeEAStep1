...
postscript("Compare_SI_DivMax_2Conf.eps", horizontal=FALSE, onefile=FALSE, height=6, width=8.5, pointsize=12)
...
NSGAIIqIndicator <- function(indicator)
{
	...
	algs<-c(expression("SI"[d], "SI"[A1],"SI"[A2]))
	boxplot(NSGAIIsi_results,NSGAIIsi_DivMax_conf1_results, NSGAIIsi_DivMax_conf3_results, names=algs, notch = FALSE)
...
SPEA2qIndicator <- function(indicator)
{
	...
	algs<-c(expression("SI"[d], "SI"[A1],"SI"[A2]))
	boxplot(SPEA2si_results,SPEA2si_DivMax_conf1_results,SPEA2si_DivMax_conf3_results, names=algs, notch = FALSE)
